...
function aiDetectsqlin($input) {
    ...
    // 调试信息（可以注释掉）
    // echo "调试 - HTTP状态码: " . $httpCode . "<br>";
    // echo "调试 - API响应: " . $response . "<br>";
...
// 用户输入
$user_input = $_GET['sqlin'] ?? '';

if ($user_input) {
    $result = aiDetectsqlin($user_input);

    if ($result && isset($result['is_sql_injection'])) {
        if ($result['is_sql_injection']) {
            // 检测到SQL注入
            echo "❌ AI 检测到 SQL 注入攻击！<br>";
            echo "消息: " . ($result['message'] ?? '未知');
        } else {
            // 安全
            echo "✅ 输入安全。<br>";
            echo "消息: " . ($result['message'] ?? '正常');
        }
    } else {
        echo "API返回格式异常";
    }
} else {
    echo "请在URL中添加 ?sqlin=你的输入";
}
```
